...
# 不可以这样使用，否则包名嵌套会报错的，common模块之间必须相互独立，
# database导包utility，utility导包reporter, reporter又导包database

class Reporter:
    def __init__(self):
        self.conn = pymysql.connect('192.168.80.128', 'root', '123456', 'test', charset='utf8')
        self.cursor = self.conn.cursor(cursor=SSDictCursor)
        # 仍旧使用woniusales数据库, 必须新建一个report表===这里会报错。。。
    ...
    def generate_report(self, version):
        ...
        for result in result_list:
            content += '\n<tr height="40">'
            content += '\n<td width="8%%">%d</td>' % result['id']
            content += '\n<td width="10%%">%s</td>' % result['module']
            content += '\n<td width="10%%">%s</td>' % result['testtype']
            ...
            content += '\n<td width="15%%">%s</td>' % result['testtime']
...
if __name__ == '__main__':
    Reporter().generate_report('2.1.2')
